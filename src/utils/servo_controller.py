...
class ServoController:
    ...
    def _initialize_servos(self):
        ...
        try:
            ...
            logging.info("Servos initialized and moved to default positions")
        except Exception as e:
    ...
    def move_to_position(self, servo_name: str, position: float, speed: Optional[float] = None):
        ...
        try:
            with self.lock:
                ...
                # Apply movement with error handling
                try:
                    servo['servo'].angle = position
                    servo['current_position'] = position
                    logging.debug(f"Servo {servo_name} moved to position {position}")
                except Exception as e:
                    logging.error(f"Failed to move servo {servo_name}: {e}")
                    raise
        except Exception as e:
            logging.error(f"Error in move_to_position for {servo_name}: {e}")
            raise
            ...
            with self.lock:
                for servo_name, servo in self.servos.items():
                    try:
                        ...
                        logging.error(f"Failed to emergency stop {servo_name}: {e}")
        except Exception as e:
        ...
        try:
            ...
            for servo in self.servos.values():
                try:
                    ...
                    logging.error(f"Failed to release servo: {e}")
            ...
            try:
                ...
                logging.error(f"Failed to deinitialize PCA9685: {e}")
        except Exception as e:
